...
\paragraph{Orientation verticale}
...
\paragraph{Reconnaissance du langage}
...
\paragraph{À l'échelle du neurone unique}
...
\paragraph{À l'échelle des réseaux, prédominance du modèle multisensoriel}
...
\subsection{Les études antérieurs et leurs limites}

\paragraph{Les études passées}
L'intégration multisensorielle a déjà été étudiée à la lumière de différentes modalités sensorielles et sur différents animaux modèle.

Dans une revue de 2008 \cite{angelaki_vestibular_2008}, les auteurs promeuvent le système vestibulaire comme particulièrement adapté à l'étude de l'intégration multisensorielle. En effet, il n'y a pas de sensation consciente du signal capté par les organes car l'intégration multimodale a lieu très tôt dans les réseaux de neurone impliqués.

À l'opposé, certain phénomènes d'intégration peuvent avoir lieu de manière consciente, certains auteurs parlent alors d'"intéraction multisensorielle" TODOcite{driver2008multisensory}. 

\paragraph{Leurs limites}
Comme on l'a vu, de nombreuses études des bases neuronales de l'intégration multisensorielle ont mis en évidence des comportement typiques pour différentes modalités sensorielles sur des animaux allant du macaque 

\paragraph{TODO Études sur le primate}
\paragraph{TODO Le poisson zèbre comme animal modèle adapté}
> motiver utilisation zebrafish
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{La larve de poisson zèbre pour l'étude de l'intégration multisensorielle}
...
Une option pour étudier le poisson dans son environnement naturel est de construire un microscope motorisé capable de suivre les mouvements du poisson lors de la nage de manière à toujours pouvoir imager le cerveau. C'est l'approche adoptée par le laboratoire RoLi TODOcite{kim2017pan-neuronal} qui peut ainsi observer certains comportements difficile à reproduire avec un poisson immobilisé. L'illumination par le côté étant impossible dans ce cas, c'est une technique de microscopie structurée développée par Jérôme Mertz TODOcite{Mertzplanar2011} qui a été utilisée.

\subsubsection{Réalité virtuelle}

\paragraph{Adaptation motrice fictive}
Ahrens \emph{et al} ont étudié cette boucle sensorimotrice dans le cas de l'OMR \cite{ahrens_brain-wide_2012}. Ils ont pour cela créé un environnement fictif dans lequel une larve paralysée est placée au dessus d'un écran. Des bandes mobiles sont présentées au poisson, ce qui déclenche le réflexe optomoteur. L'activité des neurones moteurs est enregistrée à l'aide d'électrodes (les muscles sont inactifs car le poisson est paralysé), et ce signal est utilisé pour simuler un déplacement par un mouvement des bandes en sens opposé. Dans cet environnement virtuel, ils ont pu tester des mécanismes comme l'adaptation de gain tout en enregistrant l'activité des neurones. Cela a permis d'identifier les neurones responsables de l'augmentation du gain, de la diminution du gain, essentiels pour le fonctionnement de la boucle de rétroaction.

\paragraph{TODO}

> TODO Sans rétroaction, le poisson peut décider d'arrêter de répondre aux stimuli. → give up
