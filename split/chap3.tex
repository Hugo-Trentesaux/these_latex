...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.75\textwidth]{./files/profile-intensity.png}
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.95\textwidth]{./files/pulsed_laser.svg.png}
    \caption{Profil temporel de puissance d'un laser pulsé. Chaque impulsion a une durée $\tau$, elles sont espacées d'une durée $T=1/f$, où $f$ est la fréquence de répétition du laser. À puissance moyenne $P_\text{moy}$ constante, plus $\tau$ est petit, et plus $T$ est grand, plus la puissance maximale $P_\text{max}$ est grande. L'intensité de fluorescence en deux photons est proportionnelle à la moyenne du carré de l'intensité alors qu'en un photon elle est proportionnelle à l'intensité moyenne \cite{maioli_fast_2020}.
    }
...
\subsubsection{Imagerie sur rongeur à tête fixée}\label{rongeur_fixe}
...
Pour une impulsion de 100 fs à 915 nm, cela donne un élargissement de l'ordre de 28 fs au bout d'un mètre de propagation dans la fibre. L'intensité deux photons étant inversement proportionelle à la durée d'impulsion, cela engendre une perte de signal de 22\% (1-100/128). Heureusement, il est possible de pré-compenser cette dispersion à l'aide d'un système optique placé en amont de la fibre. Le laser \emph{Mai-Tai} est justement accompagné d'un élément \emph{Deepsee} qui permet une telle précompensation réglable de -8 900 à -24 500 fs² d'après la documentation.
...
\begin{figure}
    ...
    \\ d. Pour une polarisation quasi circulaire, variation de l'ellipticité en fonction du diamètre de courbure (couleurs : répétitions de la mesure).
    \\ e. Pour une polarisation quasi linéaire, variation de l'angle de polarisation en fonction du diamètre de courbure (couleurs : répétition de la mesure).
    \label{FIGfibercarac}}
    \end{figure}
...
Le niveau de signal obtenu dans les enregistrements deux-photons est bien plus faible que dans les enregistrements un-photon. Avec les paramètres finaux (exposition, binning, puissance laser, lignées transgéniques...) la valeur d'un pixel de caméra correspondant à un neurone est de l'ordre de 2000 en un-photon et 800 en deux-photons. Le bruit est blanc (DSP constante), gaussien, centré autour de 400 (le niveau de base pour un pixel est de 100, soit 400 après binning par 4 pixels) et avec un écart type de 5. Un pixel correspondant à l'autofluorescence des tissus est autour de 600 en un-photon mais tombe dans le bruit en deux-photons. Un rapport signal à bruit si faible pose plusieurs problèmes.
...
\begin{figure}
    ...
    \\ c, d. Cartes de réponse pour une stimulation en tangage imagée en 2P. En (c.) seule la partie supérieure du cerveau est imagée, en (d.), seule la partie inférieure est imagée.
    \label{FIG1P2P}}
    \end{figure}
