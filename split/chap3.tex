...
% TODOcite \cite{belardi_effect_2013}
...
\caption{Effet de lentille thermique induite par la propagation dans l'eau d'un faisceau intense.
\\ a. Feuille de lumière imagée dans la fluorescéine pour plusieurs puissances laser différentes. La position du maximum d'intensité est marquée par un trait en pointillé.
\\ b. Simulation numérique pour les mêmes puissances. La demi largeur du faisceau est calculée pour chaque pas de propagation. La largeur minimale est marquée d'un trait pointillé.
\\ c. Comparaison de la simulation numérique et des valeurs expérimentales montrées précédemment.
\\ d. Illustration du phénomène de lentille thermique : l'indice $n$ est plus petit pour les rayons $r$ plus petits. En conséquence, des rayons focalisés en un point (gris) sont déviés (flèches noires).
...
% TODO bouger cette référence vers l'annexe
